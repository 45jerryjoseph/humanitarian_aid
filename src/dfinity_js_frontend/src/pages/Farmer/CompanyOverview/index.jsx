...
export default function CompanyOverviewPage({ farmer }) {
  ...
  const saveProduct = async (data) => {
    try {
      ...
      createProduct(data,id).then((resp) => {
        console.log(resp);
        console.log(data);
        fetchNewProducts();
  ...
  const addGrading = async (data) => {
    ...
    try {
      setLoading(true);
      await gradeProduct(data).then((resp) => {
        console.log("grade Resp", resp);
        fetchNewProducts();
        fetchGradedProducts();
      });
      toast(<NotificationSuccess text="Product graded successfully." />);
      setLoading(false);
    } catch (error) {
      console.log(error);
      setLoading(false);
      toast(<NotificationError text="Failed to grade product." />);
    }
  };
  ...
  const fetchNewProducts = useCallback(async () => {
    try {
      setLoading(true);
      setNewProducts(await getNewProductsForFarmer(id));
      setLoading(false);
    } catch (error) {
      console.log(error);
      setLoading(false);
  ...
  // get graded products
  const fetchGradedProducts = useCallback(async () => {
    try {
      setLoading(true);
      setGradedProducts(await getGradedProductsForFarmer(id));
      setLoading(false);
    } catch (error) {
      console.log(error);
      setLoading(false);
  ...
  const fetchPackagedProducts = useCallback(async () => {
    try {
      setLoading(true);
      setPackagedProducts(await getPackagedProductsForFarmer(id));
      setLoading(false);
    } catch (error) {
      console.log(error);
      setLoading(false);
  ...
  console.log("Graded", gradedProducts);
  console.log("Packaged", packagedProducts);
  console.log("ApprovedAdverts", approvedAdverts);
                        ...
                        <TabList className="flex flex-row justify-between mt-2 w-[98%] items-center p-3 bg-white-A700_01 shadow-xs rounded-[25px]">
                          ...
                          <Tab className="mt-0.5 ml-[13px] text-gray-900_01 text-[11px] font-normal">
                            <Button
                              ...
                              size="6xl"
                              className="ml-px rounded-[20px]"
                              onClick={() => setTab("ApprovedAdverts")}
                            >
                              Awaited Pickup
                            </Button>
                          </Tab>
